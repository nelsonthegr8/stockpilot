...
export default async function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeader title={`Order ${order.id.slice(-8).toUpperCase()}`} description={order.customerName ?? order.customerEmail ?? "Manual order"} back={{ href: "/dashboard/orders", label: "Orders" }} action={<Link href={`/dashboard/shipping/${order.id}`} className={buttonVariants({ variant: "outline" })}>Open shipping</Link>} />
      <div className="grid gap-4 md:grid-cols-3"><StatCard title="Status" value={order.status.replaceAll("_", " ")} hint={order.channel?.name ?? "Manual channel"} /><StatCard title="Total" value={`$${Number(order.total).toFixed(2)}`} hint={`${order.items.length} line item(s)`} /><StatCard title="Priority" value={order.priority === 2 ? "Urgent" : order.priority === 1 ? "High" : "Normal"} hint={`Raw value: ${order.priority}`} /></div>
