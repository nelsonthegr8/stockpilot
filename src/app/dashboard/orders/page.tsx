  ...
  return (
    <div className="p-6 space-y-4">
      <PageHeader
        ...
        action={["ADMIN", "MANAGER"].includes(role ?? "") ? <Link href="/dashboard/orders/new" className={buttonVariants()}><Plus className="mr-2 h-4 w-4" />New Order</Link> : undefined}
      />
      ...
      <Card><CardContent className="pt-4"><Table><TableHeader><TableRow><TableHead>Order</TableHead><TableHead>Customer</TableHead><TableHead>Channel</TableHead><TableHead>Items</TableHead><TableHead>Total</TableHead><TableHead>Priority</TableHead><TableHead>Status</TableHead><TableHead>Date</TableHead><TableHead></TableHead></TableRow></TableHeader><TableBody>{orders.map((order) => <TableRow key={order.id}><TableCell className="font-mono text-sm">{order.id.slice(-8).toUpperCase()}</TableCell><TableCell>{order.customerName ?? "—"}</TableCell><TableCell>{order.channel?.name ?? "Manual"}</TableCell><TableCell>{order.items.length}</TableCell><TableCell>${Number(order.total).toFixed(2)}</TableCell><TableCell>{order.priority === 2 ? <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">Urgent</span> : order.priority === 1 ? <span className="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800">High</span> : null}</TableCell><TableCell><span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[order.status] ?? ""}`}>{order.status.replaceAll("_", " ")}</span></TableCell><TableCell className="text-sm text-muted-foreground">{order.placedAt ? new Date(order.placedAt).toLocaleDateString() : "—"}</TableCell><TableCell><Link href={`/dashboard/orders/${order.id}`} className={buttonVariants({ variant: "ghost", size: "sm" })}>View</Link>{role === "ADMIN" ? <DeleteOrderButton orderId={order.id} orderRef={order.id.slice(-8).toUpperCase()} size="sm" /> : null}</TableCell></TableRow>)}{orders.length === 0 ? <TableRow><TableCell colSpan={9} className="text-center text-muted-foreground py-8">No orders found</TableCell></TableRow> : null}</TableBody></Table><div className="mt-4 flex justify-between text-sm text-muted-foreground"><span>Showing {total ? (page - 1) * limit + 1 : 0}–{Math.min(page * limit, total)} of {total}</span></div></CardContent></Card>
    </div>
  );
}
